...
function shortestPathToFood(boardWidth, boardHeight, head, body, food, snakes) {
  ...
  var cur = JSON.parse(JSON.stringify(food));
...
function determineDirection(boardWidth, boardHeight, head, body, food, snakes) {
  ...
  if (bestPath.length >= 1) {
    ...
    x1 = bestPath[0].x;
    y1 = bestPath[0].y;
    if (uf.getSize(uf.find(to1D(x1, y1, boardWidth, boardHeight))) > body.length) {
...
// console.log(food);
//
// var result = createUnionFindSet(gameData.board.width, gameData.board.height, gameData.you.body, gameData.board.snakes);
// var uf = result.uf;
// var cc = result.cc;
...
// console.log(s.has(101));
// console.log(s);
// console.log(cc);
// for (let c of cc) {
//   console.log(uf.getSize(c));
// }
// let sa = Array.from(s);
// let cca = Array.from(cc);
